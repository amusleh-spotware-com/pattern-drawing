...
namespace cAlgo.Helpers
{
    public static class DataSeriesAverageDistanceExtension
    {
        ...
        {
            return dataSeries.Skip(dataSeries.Count - period).Zip(dataSeries.Skip(dataSeries.Count - (period - 1)),
                (firstClose, secondClose) => Math.Abs(firstClose - secondClose)).Average();
        }
    }
}