...
namespace cAlgo.Helpers
{
    public static class SymbolTradingSessionExtensions
    {
        ...
        {
            return symbol.MarketHours.Sessions.Any(iSession =>
                (time.DayOfWeek == iSession.StartDay && time.TimeOfDay >= iSession.StartTime) ||
                (time.DayOfWeek == iSession.EndDay && time.TimeOfDay <= iSession.EndTime));
        }

        public static TimeSpan GetOutsideTradingTimeAmount(this Symbol symbol, DateTime startTime, DateTime endTime,
            TimeSpan interval)
        {
