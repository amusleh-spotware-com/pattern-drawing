...
namespace cAlgo.Helpers
{
    public static class ExceptionExtensions
    {
        ...
        {
            if (string.IsNullOrEmpty(exception.StackTrace)) return string.Empty;

            var stackTrace = new StackTrace(exception, true);

            var frames = stackTrace.GetFrames();

            if (frames == null || !frames.Any()) return string.Empty;
            ...
            foreach (var stackFram in frames)
            {
                ...
                stringBuilder.AppendFormat("{0}File: {1} | Line: {2} | Col: {3} | Offset: {4} | Method: {5}",
                    tracesIntend, fileName, stackFram.GetFileLineNumber(), stackFram.GetFileColumnNumber(),
                    stackFram.GetILOffset(), stackFram.GetMethod());
